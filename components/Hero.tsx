...
export default function Hero() {
  ...
  const waMsg = encodeURIComponent(
    "Hi, I want to book a free consultation regarding my legal matter."
  );
  ...
  return (
    <div className="relative min-h-screen bg-[#06090f] overflow-hidden flex flex-col">
          ...
          <div className={`text-white transition-opacity duration-1000 ${mounted ? "opacity-100" : "opacity-0"}`}>
            ...
            <div className="flex flex-wrap gap-4 mb-14">
              <a
                ...
                className="group relative inline-flex items-center gap-3 px-8 py-4 rounded-full font-bold text-[#06090f] overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-[0_0_30px_rgba(212,175,55,0.4)]"
                style={{ background: "linear-gradient(135deg, #d4af37, #E8C547, #d4af37)", backgroundSize: "200%" }}
              >
      ...
      {/* Scrolling marquee */}
      <div className="relative z-10 bg-[#FEFCF3] border-y border-[#e8e0cc] py-4 overflow-hidden shadow-[inset_0_4px_20px_rgba(0,0,0,0.03)]">
        <div className="flex gap-12 animate-marquee whitespace-nowrap items-center">
          {[...services, ...services].map((item, i) => (
            <span key={i} className={`text-sm font-bold ${item.length > 5 ? "text-[#0B1426] tracking-widest uppercase" : "text-lg drop-shadow-sm"}`}>
              {item}
            </span>
          ))}
        </div>
